...
path = paste(offset, 'naval-propulsion-plant/', sep="")
...
SCALE = TRUE
reg_factor = 0.0
T_STEPS = 120000 #12000 #w.r.t Batchsize Protein 75000, Boston 50000
...
spatz = 0.0
x_scale = TRUE
( out_path = paste0(path,Sys.Date(),round(rnorm(1,mean=30,sd=5),3),'_','test_run/') )
dir.create(out_path)


history = make_hist()
 for (run in 0:(runs-1)){ # loop over folds (=runs = splits)
  # run =1
