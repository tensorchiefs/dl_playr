...
grid_reg_factor = c(0.0, 0.05, 0.5)

## !! TODO: use only train for grid search, grid also over bs, x_scale?

for (reg_factor in grid_reg_factor){
  ...
  ret = get_data(path)
  (runs = ret$runs)
  for (run in 1:2 ){   # !!!!!for grid search restrict on first few folds
    #for (run in 1:runs){ #<----------------
    # run =1
    ret = get_data(path, split_num=run, spatz = 0.05, x_scale = TRUE)
  ...
  out = paste0(path,Sys.Date(),round(rnorm(1,mean=30,sd=5),3),'_','reg_factor_',reg_factor,'/')
  dir.create(out)
