...
model_beta %>% 
  ...
  #layer_dense(5, activation='thanh', input_shape = c(ncol(x))) %>% 
  layer_dense(1, activation='linear', input_shape = c(ncol(x_train))) 
summary(model_beta)
  ...
  for (r in 1:T_STEPS){
    l  = train_step_au(x=x_train, y=y_train, model_hy=model_hy,model_beta=model_beta )  
      if (r %% T_OUT == 0){
        ...
        history = rbind(history, c(r, run, l$numpy() , nll$numpy(), 'model_4'))
      } 
    }
    end_time = Sys.time() 
    if (save_model) {
      save_model_hdf5(model_hy, paste0('boston_cv_model4_hy_F',run,'_Steps_',T_STEPS))
      save_model_hdf5(model_beta, paste0('boston_cv_model4_beta_F',run,'_Steps_',T_STEPS))
    }
    return(history)
}
